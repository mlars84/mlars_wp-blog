...
function septera_continue_reading_link() {
	...
	return '<a class="continue-reading-link" href="'. esc_url( get_permalink() ) . '"><span>' . wp_kses_post( $septera_excerptcont ). '</span><i class="icon-continue-reading"></i></a>';
}
...
function septera_posted_edit() {
	edit_post_link( __( 'Edit', 'septera' ), '<span class="edit-link icon-metas"><i class="icon-edit icon-metas"></i> ', '</span>' );
}; // septera_posted_edit()
endif;
 ...
 */
if ( ! function_exists( 'septera_set_featured_thumb' ) ) :
function septera_set_featured_thumb() {

	global $post;
	$septeras = cryout_get_option( array( 'septera_fpost', 'septera_fauto', 'septera_falign', 'septera_magazinelayout', 'septera_landingpage' ) );
	...
	if ( function_exists('has_post_thumbnail') && has_post_thumbnail() && $septeras['septera_fpost']) {
		// has featured image
		$featured_image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), $featured );
	} elseif ( $septeras['septera_fpost'] && $septeras['septera_fauto'] && empty($featured_image) ) {
		...
		<div class="post-thumbnail-container"  <?php cryout_schema_microdata( 'image' ); ?>>
			...
			<a class="responsive-featured-image" href="<?php echo esc_url( get_permalink( $post->ID ) ) ?>" title="<?php echo esc_attr( get_post_field( 'post_title', $post->ID ) ) ?>">
				...
				src="<?php echo $featured_image_url; ?>" srcset="<?php if ($use_srcset) echo cryout_get_featured_srcset(
					...
					array( 	'septera-featured',
							'septera-featured-full',
							'septera-featured-half',
							'septera-featured-third' )
				) ?>" sizes="<?php if ($use_srcset) echo cryout_gen_featured_sizes( septera_featured_width(), $septeras['septera_magazinelayout'], $septeras['septera_landingpage'] ) ?>"/>
			</a>
...
} // septera_set_featured_thumb()
endif;
if ( cryout_get_option( 'septera_fpost' ) ) add_action( 'cryout_featured_hook', 'septera_set_featured_thumb' );
