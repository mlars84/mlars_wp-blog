...
function septera_enqueue_styles() {
	...
	foreach ( $cryout_theme_structure['google-font-enabled-fields'] as $item ) {
		...
		$itemw = $item . 'weight';
		// custom font names
		if ( ! empty( $septeras[$itemg] ) ) {
				$gfonts[] = cryout_gfontclean( $septeras[$itemg], ":".$septeras[$itemw] );
				$roots[] = cryout_gfontclean( $septeras[$itemg] );
		}
		// preset google fonts
		if ( preg_match('/^(.*)\/gfont$/i', $septeras[$item], $bits ) ) {
				$gfonts[] = cryout_gfontclean( $bits[1], ":".$septeras[$itemw] );
...
add_action( 'wp_head', 'septera_enqueue_styles', 5 );
